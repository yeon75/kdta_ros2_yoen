...
int main()
{
    ...
    vector<Vec2f> lines;
    HoughLines(edge, lines, 1, CV_PI / 100, 200);

    Point pt1, pt2;
    for(size_t i=0; i < lines.size(); i++)
    {
        float rho = lines[i][0], theta = lines[i][1];
        float x8 = rho * cos(theta), y8 = rho * sin(theta);
        float x1000 = 1000 * (-sin(theta));
        float y1000 = 1000 * (cos(theta)); 
        pt1.x = cvRound(x8 -x1000);
        pt1.y = cvRound(y8 -y1000);
        pt2.x = cvRound(x8 -x1000);
        pt2.y = cvRound(y8 -y1000);
